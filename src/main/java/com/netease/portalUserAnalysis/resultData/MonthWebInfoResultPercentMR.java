...
public class MonthWebInfoResultPercentMR extends MRJob {

	@Override
	public boolean init(String date) {
    	List<String> dateList;
        try {
            String firstDay = DateUtils.getTheDayBefore(date, 30);
		...
		outputList.add(DirConstant.USER_PROFILE_BASE_DIR + "monthwebInfoResultPercent/" + date);
		return true;
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = HadoopUtils.getJob(this.getClass(), this.getClass().getName() + "_step1");
		  ...
		  for (String input : inputList) {
	            ...
	            if (getHDFS().exists(path)) {
	                MultipleInputs.addInputPath(job, path, SequenceFileInputFormat.class, webStatisticInfoPercentMapper.class);
	            }
		  }
		...
		// reducer
		job.setReducerClass(webInfoResultPercentReducer.class);
	...
	public static class webStatisticInfoPercentMapper extends Mapper<Text, NullWritable, Text, Text> {
	...
	public static class webInfoResultPercentReducer extends Reducer<Text, Text, Text, Text> {
		private Text outKey = new Text();
		private Text outValue = new Text();
		...
		protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		   ...
		   float[] f = new float[statistic.length];
		   if (sum!=0) {
			   for (int i =0;i<statistic.length;i++){
				   f[i]=Float.parseFloat(statistic[i].toString())/sum;
			   }
